...
ctx.beginPath();
ctx.arc(240, 160, 70, 0, Math.PI*2, false);
ctx.fillStyle = "BLUE";
ctx.fill();
...
/*
function draw(){
    //limpia el dibujo en el canvas//
    ctx.clearRect(0,0, canvas.width, canvas.height)
    //traza la figura //
    ctx.beginPath();
    //traza un circulo con origen en x y y 
    ctx.arc( x, y, 10, 0, Math.PI*2, false);
    //guarda el color de la figura 
    ctx.fillStyle = "BLUE";
    //carga el color a la figura
    ctx.fill();
    // le dibuja la linea de contorno a la figura 
    ctx.strokeStyle = "rgba( 0, 255, 255, 0.5)",2;
    ctx.stroke();
    ctx.closePath();
    x += dx;
    y += dy;
